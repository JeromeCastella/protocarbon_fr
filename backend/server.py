from fastapi import FastAPI, HTTPException, Depends, status, UploadFile, File, APIRouter, Response
...
async def create_carbon_objective(
    ...
    # Archive any existing active objective
    ...
    # Calculate baseline from activities in reference fiscal year
    ...
    # Get all activities for this company in the reference period
    activities = list(activities_collection.find({
        "company_id": current_user["company_id"],
        "date": {"$gte": ref_start, "$lte": ref_end}
    }))
    
    # Calculate baseline by scope
    baseline_scope1 = 0
    baseline_scope2 = 0
    baseline_scope3 = 0
...
@api_router.get("/objectives/trajectory")
async def get_objective_trajectory(current_user: dict = Depends(get_current_user)):
    """Get trajectory data for charts - includes target line and actual emissions by year"""
    if not current_user.get("company_id"):
    ...
    for fy in fiscal_years:
        ...
        # Get emissions for this fiscal year
        activities = list(activities_collection.find({
            "company_id": current_user["company_id"],
            "date": {"$gte": fy_start, "$lte": fy_end}
        }))
...
@api_router.get("/objectives/recommendations")
async def get_objective_recommendations(current_user: dict = Depends(get_current_user)):
    ...
    sorted_categories = sorted(
        baseline_by_category.items(),
        key=lambda x: x[1],
        reverse=True
    )
    
    # Get top 3 categories with recommendations
    recommendations = []

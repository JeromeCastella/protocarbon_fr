...
const GeneralInfo = () => {
  ...
  return (
    <div data-testid="general-info-page" className="space-y-8 max-w-4xl">
      ...
      {/* Company Identity Card (stable data) */}
      <motion.div
      ...
      {/* Fiscal Year Context Card (data specific to selected fiscal year) */}
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.05 }}
        className={`p-6 rounded-2xl ${isDark ? 'bg-slate-800' : 'bg-white shadow-lg'} ${contextReadonly ? 'opacity-75' : ''}`}
      >
        <div className="flex items-center justify-between mb-4">
          ...
          <motion.button
            ...
            disabled={savingContext || contextReadonly || !selectedFiscalYear?.id}
            whileTap={{ scale: 0.95 }}
        ...
        {/* No fiscal year selected warning */}
        {!selectedFiscalYear?.id && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-3 rounded-xl border transition-all focus:ring-2 focus:ring-amber-500 ${
                  isDark 
                    ? 'bg-slate-700 border-slate-600 text-white' 
                    : 'bg-white border-gray-200 text-gray-900'
            ...
            <div>
              ...
              <input
                type="number"
                value={fiscalYearContext.surface_area === 0 ? '' : fiscalYearContext.surface_area}
                onChange={(e) => setFiscalYearContext({ ...fiscalYearContext, surface_area: parseFloat(e.target.value) || 0 })}
                placeholder="0"
                disabled={contextReadonly || !selectedFiscalYear?.id}
                data-testid="context-surface-input"
                className={`w-full px-4 py-3 rounded-xl border transition-all focus:ring-2 focus:ring-amber-500 ${
                  isDark 
                    ? 'bg-slate-700 border-slate-600 text-white' 
                    : 'bg-white border-gray-200 text-gray-900'
            ...
            {isPrivateCompany && (
              <div>
                ...
                <input
                  type="number"
                  value={fiscalYearContext.revenue === 0 ? '' : fiscalYearContext.revenue}
                  onChange={(e) => setFiscalYearContext({ ...fiscalYearContext, revenue: parseFloat(e.target.value) || 0 })}
                  placeholder="ex: 1500"
                  disabled={contextReadonly || !selectedFiscalYear?.id}
                  data-testid="context-revenue-input"
                  className={`w-full px-4 py-3 rounded-xl border transition-all focus:ring-2 focus:ring-amber-500 ${
                    isDark 
                      ? 'bg-slate-700 border-slate-600 text-white' 
                      : 'bg-white border-gray-200 text-gray-900'

...
const AdminExportTab = () => {
  ...
  const handleMongoDump = async () => {
    ...
    try {
      ...
      setDumpResult({
        success: true,
        message: `${t('admin.export.exportSuccess')} — ${filename} (${sizeMB} MB) — ${methodLabel}`
      });
    } catch (error) {
      logger.error('MongoDB dump error:', error);
      setDumpResult({
        success: false,
        message: `${t('admin.export.exportError')}: ${error.message}`
      });
    } finally {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      setResult({
        success: false,
        message: t('admin.export.exportError')
      });
    } finally {
      setLoading(false);
    }
  ...
  const exportTypes = [
    { 
      id: 'full', 
      label: t('admin.export.fullBackup'),
      description: t('admin.export.fullBackupDesc'),
      icon: Database,
      color: 'blue'
    },
    { 
      id: 'reference-data', 
      label: t('admin.export.referenceData'),
      description: t('admin.export.referenceDataDesc'),
      icon: FileCode,
      color: 'orange'
    },
    { 
      id: 'emission-factors', 
      label: t('admin.export.emissionFactors'),
      description: t('admin.export.emissionFactorsDesc'),
      icon: Layers,
      color: 'yellow'
    },
    { 
      id: 'activities', 
      label: t('admin.export.activities'),
      description: t('admin.export.activitiesDesc'),
      icon: Activity,
      color: 'green'
    },
    { 
      id: 'products', 
      label: t('admin.export.products'),
      description: t('admin.export.productsDesc'),
      icon: Package,
      color: 'purple'
    }
  ];
      ...
      <div className={`p-6 rounded-xl border-2 ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'} shadow-sm`}>
        <div className="flex items-start gap-4 mb-5">
          ...
          <div className="flex-1">
            <h3 className={`text-lg font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>
              {t('admin.export.mongoExport')}
            </h3>
            <p className={`text-sm mt-1 ${isDark ? 'text-slate-400' : 'text-gray-500'}`}>
              {t('admin.export.mongoExportDesc')}
            </p>
          </div>
        </div>
